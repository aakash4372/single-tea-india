...
const GalleryPage = () => {
  ...
  const handleEdit = async (e) => {
    e.preventDefault();
    if (!selectedFile) {
      showToast("error", "Please select a new image");
      return;
    }
    ...
    try {
      const response = await axios.put(
        `${import.meta.env.VITE_BASE_URL}/gallery/${editItemId}`,
        formData,
        {
          headers: { "Content-Type": "multipart/form-data" },
        }
      ...
      showToast("error", err.response?.data?.message || "Failed to update image");
    } finally {
      setIsUploading(false);
    }
  };
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`${import.meta.env.VITE_BASE_URL}/gallery/${deleteItemId}`);
      setGalleries(galleries.filter((item) => item._id !== deleteItemId));
        ...
        <div className="fixed inset-0 bg-black/70 flex justify-center items-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 border border-gray-100 animate-fade-in relative">
            ...
            <h2 className="text-2xl font-bold mb-6 text-gray-800 flex items-center gap-2">
              <span>📸</span> Upload Gallery Image
            </h2>
      ...
      {isEditModalOpen && (
        <div className="fixed inset-0 bg-black/70 flex justify-center items-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 border border-gray-100 animate-fade-in relative">
            ...
            <h2 className="text-2xl font-bold mb-6 text-gray-800 flex items-center gap-2">
              <span>✏️</span> Edit Gallery Image
            </h2>
